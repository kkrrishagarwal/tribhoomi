...
def record(db: Session, *, actor: str, role: str, action: str, unit: Unit | None = None, parcel_id: int | None = None,
           ...
           when: datetime | None = None) -> AuditEvent:
    ev = AuditEvent(
        ...
        subject=subject or (unit.tpid if unit else ""), previous_value=previous, new_value=new, status=status, note=note,
    )
    db.add(ev)
    db.flush()
    return ev
...
def to_dict(ev: AuditEvent) -> dict:
    return {
        ...
        "new_value": ev.new_value, "status": ev.status, "note": ev.note, "unit_id": ev.unit_id, "parcel_id": ev.parcel_id,
    }
