...
def create_layout(
    ...
    *,
    ...
    b_no = 1
    bld = Building(
        parcel_id=parcel.id, building_no=b_no, building_ulpin=make_3d_ulpin(key, b_no, 0, 0),
        name=spec.get("name") or name, num_floors=int(spec["num_floors"]), num_basements=int(spec.get("num_basements", 0)),
    ...
    units_per_floor = int(spec["units_per_floor"])
    ...
    for fno in list(range(-bld.num_basements, 0)) + list(range(0, bld.num_floors)):
        ...
        floor = Floor(building_id=bld.id, floor_number=fno, floor_ulpin=make_3d_ulpin(key, b_no, level, 0),
                      height_m=height, base_elevation_m=round(base, 2),
                      layer_type=(LayerType.UNDERGROUND if fno < 0 else LayerType.ABOVE_GROUND).value)
        db.add(floor)
        db.flush()
        n = 1 if fno < 0 and usage in ("parking", "utility") else units_per_floor
        col_w = (maxx - minx) / n
        for u in range(1, n + 1):
            ...
            db.add(Unit(
                floor_id=floor.id, unit_no=u, unit_ulpin=make_3d_ulpin(key, b_no, level, u),
