    ...
    <div className="mx-auto grid h-[calc(100vh-10.25rem)] min-h-[520px] max-w-screen-2xl grid-cols-1 gap-4 p-4 lg:grid-cols-[400px_1fr]">
      ...
      <section className="card relative overflow-hidden">
        <ParcelMap
          ...
          candidates={t.candidates} handleRef={mapRef} market={showMarket ? t.market : []}
        />
        <div className="absolute bottom-6 left-3 z-[1000] flex flex-col items-start gap-1">{showMarket && <MapLegend />}<label className="glass flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-xs"><input type="checkbox" checked={showMarket} onChange={(e) => setShowMarket(e.target.checked)} /> Show real NCR context <span className="text-ink-dim">(RERA-listed projects, informational)</span></label></div>
        {t.drawing && (
