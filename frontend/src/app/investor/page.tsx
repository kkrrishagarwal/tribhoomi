...
export default function InvestorPage() {
  ...
  useEffect(() => { if (s.role === "investor" || s.role === "admin") load(); }, [s]);

  if (s.role !== "investor" && s.role !== "admin") return <div className="mx-auto max-w-lg p-8 text-center text-sm text-slate-600">Switch to an <b>Investor</b> identity from the header dropdown to see your plots.</div>;
  ...
  return (
    <div className="mx-auto max-w-screen-xl space-y-4 p-4">
      <div>
        <h1 className="text-xl font-semibold">{t("h.investor")} · {s.name}</h1>
