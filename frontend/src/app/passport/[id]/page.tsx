...
import { api, type Analysis, type PropertyPage } from "@/lib/api";
...
export default function Passport() {
  ...
  const [p, setP] = useState<PropertyPage | null>(null); const [err, setErr] = useState<string | null>(null); const [an, setAn] = useState<Analysis | null>(null);
  useEffect(() => { api.property(id).then(setP).catch((e) => setErr(e.message)); api.analysis(id).then(setAn).catch(() => null); }, [id]);
  ...
  return (
    <div className="page"><div className="card mx-auto max-w-md p-6">
      ...
      <div className="mt-4 space-y-2 text-sm">
        <div className="rounded-lg bg-slate-50 p-3"><div className="text-slate-500">Is this property registered in Tribhoomi?</div><div className="text-lg font-bold text-emerald-300">Yes</div></div>
        <div className="rounded-lg bg-slate-50 p-3"><div className="text-slate-500">Is its current version verified?</div><div className={`text-lg font-bold ${p.verification_status === "verified" ? "text-emerald-300" : p.verification_status === "pending" ? "text-amber-300" : "text-red-300"}`}>{p.verification_status === "verified" ? "Verified" : p.verification_status === "pending" ? "Pending" : "Review required"}</div></div>
        <div className="rounded-lg bg-slate-50 p-3"><div className="text-slate-500">Has Tribhoomi detected a current spatial conflict?</div><div className={`text-lg font-bold ${an ? (an.trust.geometry.ok ? "text-emerald-300" : "text-red-300") : ""}`}>{an ? (an.trust.geometry.ok ? "No conflict" : "Potential conflict") : "…"}</div></div>
      </div>
      ...
      <div className="mt-4 flex gap-2"><Link href={`/property/${p.tpid}`} className="btn-primary flex-1 justify-center">View property history</Link></div>
      <p className="mt-3 text-xs text-slate-500">This page shows verification information only. Owner details and internal authority notes are never shown publicly.</p>
    </div></div>
  );
}
